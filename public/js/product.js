...
$(document).ready(function () {
    ...
    $(document).on('change', 'input#single_dpp', function (e) {
        ...
        var commission_percent = __read_number($('#commission_percent'));
        commission_percent = commission_percent == undefined ? 0 : commission_percent;

        var purchase_price = __substract_percent(purchase_exc_tax, commission_percent);
        __write_number($('input#single_dpp'), purchase_price);

        var purchase_price_inc_tax = __substract_percent(purchase_price, tax_rate);
        __write_number($('input#single_dpp_inc_tax'), purchase_price_inc_tax);

        var selling_price = __add_percent(purchase_price, profit_percent);
    ...
    $(document).on('change', 'input#commission_percent', function (e) {
        var tax_rate = $('select#tax').find(':selected').data('rate');
        tax_rate = tax_rate == undefined ? 0 : tax_rate;
        ...
        var commission_percent = __read_number($('input#commission_percent'));
    ...
    $(document).on('change', 'input.variable_dpp', function (e) {
        ...
        var commission_percent = __read_number(tr_obj.find('input.variable_commission_percent'));
        commission_percent = commission_percent == undefined ? 0 : commission_percent;

        var purchase_price = __substract_percent(purchase_exc_tax, commission_percent);
        __write_number(tr_obj.find('input.variable_dpp'), purchase_price);

        var purchase_price_inc_tax = __substract_percent(purchase_price, tax_rate);
        __write_number(tr_obj.find('input.variable_dpp_inc_tax'), purchase_price_inc_tax);
    ...
    $(document).on('change', 'input.variable_commission_percent', function (e) {
        var tax_rate = $('select#tax').find(':selected').data('rate');
        tax_rate = tax_rate == undefined ? 0 : tax_rate;

        var tr_obj = $(this).closest('tr');
        var commission_percent = __read_number($(this));
...
$(document).on('click', 'button.apply-all', function () {
    ...
    var buttonRow = $(this).closest('tr'); // Get the row that contains the apply all button

    $(this)
        .closest('tbody')
        .find('tr')
        .not(buttonRow) // Exclude the row with the button
        .each(function () {
