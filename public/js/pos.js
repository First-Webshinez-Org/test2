$(document).ready(function () {
    ...
    //Update pos quantity and trigger change event of pos_quantity
    $('table#pos_table tbody').on('change', 'input.input_sub_unit', function () {
        $('input.pos_quantity').trigger('change');
    });

    // Setup an event listener on select elements with class 'imei_number'
    $('table#pos_table tbody').on('change', 'select.imei_number', function () {
        var $select = $(this);
        ...
        var matches = inputId.match(/products\[(\d+)\]\[imei_no_line_id\]\[\]/);
        if (matches && matches.length > 1) {
            var row_count = matches[1];
        }

        var quantitySelector = `products[${row_count}][quantity]`;
        document.getElementById(quantitySelector).value = selectedCount;

        var tr = $(this).parents('tr');

        var unit_price_inc_tax = __read_number(tr.find('input.pos_unit_price_inc_tax'));
        var line_total = parseFloat(selectedCount) * unit_price_inc_tax;
    ...
    $('table#pos_table tbody').on(
        'change',
        'select.row_charge_type, input.row_charge_amount',
        function () {
            var tr = $(this).parents('tr');
    ...
    $(
        'select#product_category, select#product_brand, select#product_variation, select#select_location_id'
...
function calculate_discounted_unit_price(row) {
    ...
    // var row_charge_type = row.find("select.row_charge_type").val();
    // var row_charge_amount = __read_number(row.find("input.row_charge_amount"));
    // if (row_charge_amount) {
    //   if (row_charge_type == "fixed") {
    //     row_discounted_unit_price = row_discounted_unit_price + row_charge_amount;
    //   } else {
    //     row_discounted_unit_price = __add_percent(
    //       row_discounted_unit_price,
    //       row_charge_amount
    //     );
    //   }
    // }
...
$('div#product_list_body').on('scroll', function () {
    if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
        ...
        var variation_id = $('select#product_variation').val();
...
function isValidatRewardPoint() {
    ...
    if (points > max_points) {
        is_valid = false;
        msg = __translate('max_rp_reached_error', {
