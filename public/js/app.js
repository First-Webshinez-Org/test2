$(document).ready(function () {
    ...
    // register checkbox product purchase details
    $(document).ready(function () {
        var isChecked = $('#product_purchase_details').is(':checked');
        if (isChecked) {
            $('#product_purchase_details_div').show();
        } else {
            $('#product_purchase_details_div').hide();
        }

        $('#product_purchase_details').on('ifChanged', function (event) {
            if (this.checked) {
                $('#product_purchase_details_div').show();
            } else {
                $('#product_purchase_details_div').hide();
    ...
    // register checkbox product details
    $(document).ready(function () {
        ...
        $('#product_details').on('ifChanged', function (event) {
            if (this.checked) {
                $('#product_details_div').show();
            } else {
                $('#product_details_div').hide();
    ...
    // register checkbox product details brand
    $(document).ready(function () {
        var isChecked = $('#product_details_brand').is(':checked');
        if (isChecked) {
            $('#product_details_brand_div').show();
        } else {
            $('#product_details_brand_div').hide();
        }

        $('#product_details_brand').on('ifChanged', function (event) {
            if (this.checked) {
                $('#product_details_brand_div').show();
            } else {
                $('#product_details_brand_div').hide();
    ...
    // register checkbox service details
    $(document).ready(function () {
        var isChecked = $('#service_details').is(':checked');
        if (isChecked) {
        ...
        $('#service_details').on('ifChanged', function (event) {
            if (this.checked) {
                $('#service_details_div').show();
            } else {
                $('#service_details_div').hide();
    ...
    // register checkbox service details
    $(document).ready(function () {
        var isChecked = $('#payments_details').is(':checked');
        if (isChecked) {
            $('#payments_details_div').show();
        } else {
            $('#payments_details_div').hide();
        }

        $('#payments_details').on('ifChanged', function (event) {
            if (this.checked) {
                $('#payments_details_div').show();
            } else {
                $('#payments_details_div').hide();
    ...
    // added (start) (dev_ma) to line: 542
    //Start: CRUD for mother unit
    //Mother Unit table
    var mother_units_table = $('#mother_unit_table').DataTable({
        processing: true,
        serverSide: true,
        //start crud for mother units dev-nu
        ajax: function (data, callback, settings) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    // Don't forget to call the callback function provided by DataTables
                    // to render the table with the data fetched
                    callback(response);
                },
    ...
    //dev-nu start
    $(document).on('submit', 'form#mother_unit_add_form', function (e) {
        e.preventDefault();
        var form = $(this);
        var data = form.serialize();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success == true) {
                    $('div.mother_unit_modal').modal('hide');
                    toastr.success(result.msg);
                    mother_units_table.ajax.reload();
                } else {
                    toastr.error(result.msg);
                }
            },
    ...
    $(document).on('click', 'button.edit_mother_unit_button', function () {
        $('div.mother_unit_modal').load($(this).data('href'), function () {
            $(this).modal('show');

            $('form#mother_unit_edit_form').submit(function (e) {
                e.preventDefault();
                var form = $(this);
                var data = form.serialize();
    ...
    $(document).on('click', 'button.delete_mother_unit_button', function () {
        swal({
            title: LANG.sure,
            text: LANG.confirm_delete_mother_unit,
            icon: 'warning',
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
    ...
    // added (end) (dev_ma) from line: 463
    ...
    if (contact_table_type == 'supplier') {
        var columns = [
            ...
            { data: 'final_due', orderable: false},
    ...
    contact_table = $('#contact_table').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                ...
                if (
                    $('#has_purchase_return').length > 0 &&
                    $('#has_purchase_return').is(':checked')
                ) {
                    d.has_purchase_return = true;
                }

                if (
                    $('#has_advance_balance').length > 0 &&
                    $('#has_advance_balance').is(':checked')
                ) {
                    d.has_advance_balance = true;
                }

                if (
                    $('#has_opening_balance').length > 0 &&
                    $('#has_opening_balance').is(':checked')
                ) {
                    d.has_opening_balance = true;
                }

                if (
                    $('#has_final_due').length > 0 &&
                    $('#has_final_due').is(':checked')
                ) {
                    d.has_final_due = true;
                }
        ...
        footerCallback: function (row, data, start, end, display) {
            ...
            for (var r in data) {
                total_balance += $(data[r].balance).data('orig-value')
                    ? parseFloat($(data[r].balance).data('orig-value'))
                    : 0;

                total_opening_balance += $(data[r].opening_balance).data('orig-value')
                    ? parseFloat($(data[r].opening_balance).data('orig-value'))
                    : 0;
                ...
                final_due += $(data[r].final_due).data('orig-value')
                    ? parseFloat($(data[r].final_due).data('orig-value'))
                    : 0;
            }
            $('.footer_balance').html(__currency_trans_from_en(total_balance));
            $('.footer_opening_balance').html(__currency_trans_from_en(total_opening_balance));
            ...
            $('.footer_final_due').html(__currency_trans_from_en(final_due));
        },
    });

    $(document).on(
        'ifChanged',
        '#has_sell_due, #has_sell_return, \
      #has_purchase_due, #has_purchase_return, #has_advance_balance, #has_opening_balance, #has_final_due',
        function () {
    ...
    $(document).on(
        'change',
        '#has_no_sell_from, #cg_filter, #status_filter, #assigned_to',
        function () {
            contact_table.ajax.reload();
        }
    );
    ...
    $(document).on(
        'change',
        'input[type=radio][name=scheme_type], select#invoice_number_type',
        function () {
    ...
    $(
        'select#location_id, select#expense_for, select#expense_contact_filter, \
          ...
          select#expense_sub_category_id_filter'
    ).on('change', function () {
        expense_table.ajax.reload();
    });
    ...
    $(document).on('change', '.payment_types_dropdown', function () {
        ...
        if (
            $(this).closest('.payment_row').find('.enable_cash_denomination_for_payment_methods')
                .length
        ) {
            var payment_methods = JSON.parse(
                $(this)
                    .closest('.payment_row')
                    .find('.enable_cash_denomination_for_payment_methods')
                    .val()
            );
...
function updateStockBySellingPrice(data) {
    if ($('#closing_stock_by_sp').length > 0) {
        $.ajax({
            ...
            data: data,
            success: function (result) {
                $('#closing_stock_by_sp').html(
                    __currency_trans_from_en(result.closing_stock_by_sp),
                    true
                );
                $('#opening_stock_by_sp').html(
                    __currency_trans_from_en(result.opening_stock_by_sp),
                    true
                );
            },
        });
...
$(document).on(
    'change',
    'input#expense_final_total, #add_expense_modal_form .payment-amount',
    function () {
...
$(document).on('submit', 'form#pay_contact_due_form', function (e) {
    ...
    var denomination_for_payment_types = JSON.parse(
        $('#pay_contact_due_form .enable_cash_denomination_for_payment_methods').val()
    );
    if (
        denomination_for_payment_types.includes(payment_type) &&
        $('#pay_contact_due_form .is_strict').length &&
        $('#pay_contact_due_form .is_strict').val() === '1'
    ) {
        var payment_amount = __read_number($('#pay_contact_due_form .payment_amount'));
        var total_denomination = $('#pay_contact_due_form')
