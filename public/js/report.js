$(document).ready(function () {
  ...
  supplier_report_tbl = $("#supplier_report_tbl").DataTable({
    ...
    fnDrawCallback: function (oSettings) {
      ...
      var total_sell = sum_table_col(
        $("#supplier_report_tbl"),
        "total_invoice"
      );
      $("#footer_total_sell").text(total_sell);

      var total_sell_return = sum_table_col(
        $("#supplier_report_tbl"),
        "total_sell_return"
      );
    ...
    createdRow: function (row, data, dataIndex) {
      if (
        $("#has_sell_due2").length > 0 &&
        $("#has_sell_due2").is(":checked")
      ) {
        var dueColumnValue = $(row).find("td:eq(6)").text();
        if (dueColumnValue.split(" ")[1] === "0.00") {
          $(row).hide();
        }
      }
    },
  ...
  if ($("#supplier_report_tbl").length != 0) {
    ...
    $(document).on("ifChanged", "#has_sell_due2", function () {
      supplier_report_tbl.ajax.reload();
    });
  }
  ...
  stock_report_table = $("#stock_report_table").DataTable({
    ...
    fnDrawCallback: function (oSettings) {
      ...
      $(".footer_total_stock").html(
        __sum_stock($("#stock_report_table"), "current_stock")
      );
    },
    footerCallback: function (row, data, start, end, display) {
      ...
      for (var r in data) {
        ...
        total_stock_price += $(data[r].stock_price).data("orig-value")
          ? parseFloat($(data[r].stock_price).data("orig-value"))
          : 0;

        footer_stock_value_by_sale_price += $(
          data[r].stock_value_by_sale_price
        ).data("orig-value")
        ...
        total_potential_profit += $(data[r].potential_profit).data("orig-value")
          ? parseFloat($(data[r].potential_profit).data("orig-value"))
          : 0;
      ...
      $(".footer_total_stock_price").html(
        __currency_trans_from_en(total_stock_price)
      );
      ...
      $(".footer_potential_profit").html(
        __currency_trans_from_en(total_potential_profit)
      );
      if ($("th.current_stock_mfg").length) {
        $(".footer_total_mfg_stock").html(
          __currency_trans_from_en(footer_total_mfg_stock, false)
        );
      }
    },
  });
  ...
  register_report_table = $("#register_report_table").DataTable({
    ...
    columns: [
      ...
      {
        data: "total_cheque_payment",
        name: "total_cheque_payment",
        searchable: false,
      },
      {
      ...
      {
        data: "total_bank_transfer_payment",
        name: "total_bank_transfer_payment",
        searchable: false,
      },
      {
        data: "total_advance_payment",
        name: "total_advance_payment",
        searchable: false,
      },
      {
        data: "total_custom_pay_1",
        name: "total_custom_pay_1",
        searchable: false,
      },
      {
        data: "total_custom_pay_2",
        name: "total_custom_pay_2",
        searchable: false,
      },
      {
        data: "total_custom_pay_3",
        name: "total_custom_pay_3",
        searchable: false,
      },
      {
        data: "total_custom_pay_4",
        name: "total_custom_pay_4",
        searchable: false,
      },
      {
        data: "total_custom_pay_5",
        name: "total_custom_pay_5",
        searchable: false,
      },
      {
        data: "total_custom_pay_6",
        name: "total_custom_pay_6",
        searchable: false,
      },
      {
        data: "total_custom_pay_7",
        name: "total_custom_pay_7",
        searchable: false,
      },
      {
        data: "total_other_payment",
    ...
    footerCallback: function (row, data, start, end, display) {
      ...
      for (var r in data) {
        total_card_payment += $(data[r].total_card_payment).data("orig-value")
          ? parseFloat($(data[r].total_card_payment).data("orig-value"))
          : 0;

        total_cheque_payment += $(data[r].total_cheque_payment).data(
          "orig-value"
        )
          ? parseFloat($(data[r].total_cheque_payment).data("orig-value"))
          : 0;

        total_cash_payment += $(data[r].total_cash_payment).data("orig-value")
          ? parseFloat($(data[r].total_cash_payment).data("orig-value"))
          : 0;

        total_bank_transfer_payment += $(
          data[r].total_bank_transfer_payment
        ).data("orig-value")
          ? parseFloat(
              $(data[r].total_bank_transfer_payment).data("orig-value")
            )
          : 0;

        total_other_payment += $(data[r].total_other_payment).data("orig-value")
          ? parseFloat($(data[r].total_other_payment).data("orig-value"))
          : 0;

        total_advance_payment += $(data[r].total_advance_payment).data(
          "orig-value"
        )
          ? parseFloat($(data[r].total_advance_payment).data("orig-value"))
          : 0;

        total_custom_pay_1 += $(data[r].total_custom_pay_1).data("orig-value")
          ? parseFloat($(data[r].total_custom_pay_1).data("orig-value"))
          : 0;

        total_custom_pay_2 += $(data[r].total_custom_pay_2).data("orig-value")
          ? parseFloat($(data[r].total_custom_pay_2).data("orig-value"))
          : 0;

        total_custom_pay_3 += $(data[r].total_custom_pay_3).data("orig-value")
          ? parseFloat($(data[r].total_custom_pay_3).data("orig-value"))
          : 0;

        total_custom_pay_4 += $(data[r].total_custom_pay_4).data("orig-value")
          ? parseFloat($(data[r].total_custom_pay_4).data("orig-value"))
          : 0;

        total_custom_pay_5 += $(data[r].total_custom_pay_5).data("orig-value")
          ? parseFloat($(data[r].total_custom_pay_5).data("orig-value"))
          : 0;

        total_custom_pay_6 += $(data[r].total_custom_pay_6).data("orig-value")
          ? parseFloat($(data[r].total_custom_pay_6).data("orig-value"))
          : 0;

        total_custom_pay_7 += $(data[r].total_custom_pay_7).data("orig-value")
          ? parseFloat($(data[r].total_custom_pay_7).data("orig-value"))
          : 0;

        total += $(data[r].total).data("orig-value")
          ? parseFloat($(data[r].total).data("orig-value"))
          : 0;
      }

      $(".footer_total_card_payment").html(
        __currency_trans_from_en(total_card_payment)
      );
      $(".footer_total_cheque_payment").html(
        __currency_trans_from_en(total_cheque_payment)
      );
      $(".footer_total_cash_payment").html(
        __currency_trans_from_en(total_cash_payment)
      );
      $(".footer_total_bank_transfer_payment").html(
        __currency_trans_from_en(total_bank_transfer_payment)
      );
      $(".footer_total_other_payments").html(
        __currency_trans_from_en(total_other_payment)
      );
      $(".footer_total_advance_payment").html(
        __currency_trans_from_en(total_advance_payment)
      );
      $(".footer_total_custom_pay_1").html(
        __currency_trans_from_en(total_custom_pay_1)
      );
      $(".footer_total_custom_pay_2").html(
        __currency_trans_from_en(total_custom_pay_2)
      );
      $(".footer_total_custom_pay_3").html(
        __currency_trans_from_en(total_custom_pay_3)
      );
      $(".footer_total_custom_pay_4").html(
        __currency_trans_from_en(total_custom_pay_4)
      );
      $(".footer_total_custom_pay_5").html(
        __currency_trans_from_en(total_custom_pay_5)
      );
      $(".footer_total_custom_pay_6").html(
        __currency_trans_from_en(total_custom_pay_6)
      );
      $(".footer_total_custom_pay_7").html(
        __currency_trans_from_en(total_custom_pay_7)
      );
  ...
  if ($("#sr_date_filter").length == 1) {
    //date range setting
    $("input#sr_date_filter").daterangepicker(
      dateRangeSettings,
      function (start, end) {
    ...
    if ($("#sr_payments_with_commission_table").length > 0) {
      sr_payments_with_commission_report = $(
        "table#sr_payments_with_commission_table"
      ).DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[1, "desc"]],
        ajax: {
          ...
          data: function (d) {
            ...
            d.commission_agent =
              commission_agent != "" ? $("select#sr_id").val() : 0;
        ...
        fnDrawCallback: function (oSettings) {
          var total_amount = sum_table_col(
            $("#sr_payments_with_commission_table"),
            "paid-amount"
          );
    ...
    sr_sales_report = $("table#sr_sales_report").DataTable({
      ...
      fnDrawCallback: function (oSettings) {
        ...
        $("#sr_footer_total_paid").text(
          sum_table_col($("#sr_sales_report"), "total-paid")
        );
    ...
    sr_expenses_report = $("table#sr_expenses_report").DataTable({
      ...
      fnDrawCallback: function (oSettings) {
        var expense_total = sum_table_col(
          $("#sr_expenses_report"),
          "final-total"
        );
    ...
    sr_sales_commission_report = $(
      "table#sr_sales_with_commission_table"
    ).DataTable({
      processing: true,
      serverSide: true,
      aaSorting: [[0, "desc"]],
      ...
      fnDrawCallback: function (oSettings) {
        ...
        $("#footer_payment_status_count ").html(
          __sum_status_html(
            $("#sr_sales_with_commission_table"),
  ...
  product_purchase_report = $("table#product_purchase_report_table").DataTable({
    ...
    fnDrawCallback: function (oSettings) {
      ...
      $("#footer_purchase_tax").text(
        sum_table_col($("#product_purchase_report_table"), "tax")
      );

      $("#footer_purchase_discount_amount").text(
        sum_table_col($("#product_purchase_report_table"), "discount_amount")
      );
    ...
    product_sell_report = $("table#product_sell_report_table").DataTable({
      ...
      columns: [
        ...
        { data: "sub_sku", name: "v.sub_sku" },
        {
          data: "product_custom_field1",
          name: "p.product_custom_field1",
          visible: $("#psr_product_custom_field1").html().trim().length > 0,
        },
        {
          data: "product_custom_field2",
          name: "p.product_custom_field2",
          visible: $("#psr_product_custom_field2").html().trim().length > 0,
        },
      ...
      fnDrawCallback: function (oSettings) {
        ...
        $("#footer_tax").text(
          sum_table_col_report($("#product_sell_report_table"), "tax")
        );

        $("#footer_discount_amount").text(
          sum_table_col_report(
  ...
  product_sell_report_with_purchase_table = $(
    "table#product_sell_report_with_purchase_table"
  ).DataTable({
    processing: true,
    serverSide: true,
    aaSorting: [[4, "desc"]],
  ...
  product_sell_grouped_report = $(
    "table#product_sell_grouped_report_table"
  ).DataTable({
    processing: true,
    serverSide: true,
    aaSorting: [[1, "desc"]],
  ...
  lot_report = $("table#lot_report").DataTable({
    ...
    fnDrawCallback: function (oSettings) {
      $("#footer_total_stock").html(
        __sum_stock($("#lot_report"), "total_stock")
      );
      $("#footer_total_sold").html(__sum_stock($("#lot_report"), "total_sold"));
      $("#footer_total_adjusted").html(
        __sum_stock($("#lot_report"), "total_adjusted")
      );
  ...
  if ($("table#lot_report").length == 1) {
    $("#location_id, #category_id, #sub_category_id, #unit, #brand").change(
      function () {
        lot_report.ajax.reload();
      }
    );
  ...
  $("#purchase_payment_report_table tbody").on(
    "click",
  ...
  $(
    "#sell_payment_report_form #location_id, #sell_payment_report_form #customer_id, #sell_payment_report_form #payment_types, #sell_payment_report_form #customer_group_filter"
  ).change(function () {
    sell_payment_report.ajax.reload();
  });
  ...
  if ($("#ir_purchase_date_filter").length == 1) {
    $("#ir_purchase_date_filter").daterangepicker(
      dateRangeSettings,
      function (start, end) {
  ...
  if ($("#ir_sale_date_filter").length == 1) {
    $("#ir_sale_date_filter").daterangepicker(
      dateRangeSettings,
      function (start, end) {
  ...
  items_report_table = $("#items_report_table").DataTable({
    ...
    fnDrawCallback: function (oSettings) {
      $("#footer_total_pp").html(
        sum_table_col($("#items_report_table"), "purchase_price")
      );
  ...
  $(document).on(
    "change",
    "#ir_supplier_id, #ir_customer_id, #ir_location_id",
    function () {
      items_report_table.ajax.reload();
    }
  );
  ...
  $(
    "#tax_report_location_id, #tax_report_date_range, #tax_report_contact_id"
  ).change(function () {
    updateTaxReport();
  });
});
...
function updatePurchaseSell() {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $(".purchase_due").html(
        __currency_trans_from_en(data.purchase.purchase_due, true)
      );

      $(".total_sell").html(
        __currency_trans_from_en(data.sell.total_sell_exc_tax, true)
      );
      $(".sell_inc_tax").html(
        __currency_trans_from_en(data.sell.total_sell_inc_tax, true)
      );
      $(".sell_due").html(
        __currency_trans_from_en(data.sell.invoice_due, true)
      );
      $(".purchase_return_inc_tax").html(
        __currency_trans_from_en(data.total_purchase_return, true)
      );
      $(".total_sell_return").html(
        __currency_trans_from_en(data.total_sell_return, true)
      );

      $(".sell_minus_purchase").html(
        __currency_trans_from_en(data.difference.total, true)
      );
      __highlight(data.difference.total, $(".sell_minus_purchase"));

      $(".difference_due").html(
        __currency_trans_from_en(data.difference.due, true)
      );
...
function updateStockAdjustmentReport() {
  ...
  $.ajax({
    ...
    success: function (data) {
      $(".total_amount").html(
        __currency_trans_from_en(data.total_amount, true)
      );
      $(".total_recovered").html(
        __currency_trans_from_en(data.total_recovered, true)
      );
      $(".total_normal").html(
        __currency_trans_from_en(data.total_normal, true)
      );
      $(".total_abnormal").html(
        __currency_trans_from_en(data.total_abnormal, true)
      );
    },
  });
...
function salesRepresentativeTotalSales() {
  ...
  $.ajax({
    ...
    data: data_expense,
    success: function (data) {
      $("span#sr_total_sales").html(
        __currency_trans_from_en(data.total_sell_exc_tax, true)
      );
      $("span#sr_total_sales_return").html(
        __currency_trans_from_en(data.total_sell_return_exc_tax, true)
      );
      $("span#sr_total_sales_final").html(
        __currency_trans_from_en(data.total_sell, true)
      );
    },
  });
}
      ...
      success: function (data) {
        ...
        if (data.commission_percentage != 0) {
          ...
          if (data.total_payment_with_commission) {
            ...
            $("span#total_payment_with_commsn").html(
...
function get_stock_value() {
  ...
  $.ajax({
    ...
    data: data,
    success: function (data) {
      $("#closing_stock_by_pp").text(
        __currency_trans_from_en(data.closing_stock_by_pp)
      );
      $("#closing_stock_by_sp").text(
        __currency_trans_from_en(data.closing_stock_by_sp)
      );
      $("#potential_profit").text(
        __currency_trans_from_en(data.potential_profit)
      );
      $("#profit_margin").text(
        __currency_trans_from_en(data.profit_margin, false)
      );
    },
  });
}
