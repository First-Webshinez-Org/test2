...
function sum_table_col_report(table, class_name) {
    var sum = 0;
    var processedInvoices = {}; // Object to keep track of processed invoice numbers

    table
        .find('tbody')
        .find('tr')
        .each(function () {
            var invoiceNo = $(this).find('.invoice_no').text();

            if (processedInvoices.hasOwnProperty(invoiceNo)) {
                return;
            }

            // Mark the current invoice number as processed
