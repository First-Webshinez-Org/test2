@foreach( $variations as $variation)
    <tr data-warranties="{{ json_encode($warranties) }}" @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        @if(session('business.enable_imei_number'))
            @php
                $imei_number = !empty($imported_data['imei_number']) ? $imported_data['imei_number'] : null;
            @endphp
            <td class="@if($product->enable_sr_no == 1) purchase_imei_inputs_container @endif">
                {!! Form::text('purchases[' . $row_count . '][imei_number][0]', $imei_number, ['class' => 'form-control input-sm']); !!}
            </td>
        @endif
        @if($is_warranty_enabled)
            @php
                $warranty_date = !empty($imported_data['warranty_date']) ? $imported_data['warranty_date'] : null;
            @endphp
            <td class="@if($product->enable_sr_no == 1) purchase_warranty_container @endif">
                {!! Form::select('purchases[' . $row_count . '][warranty_date][0]', $warranties, null , ['class' => 'form-control select2', 'style' => 'width: 100% !important;', 'placeholder' => __('messages.please_select')]); !!}
            </td>
        @endif
