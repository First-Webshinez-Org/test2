...
<div class="table-responsive">
    ...
    @foreach($purchase->purchase_lines as $purchase_line)
        @php
            $imei_with_warranty = !empty($purchase_line->imei_with_warranty) ? unserialize($purchase_line->imei_with_warranty) : null;

            $imei_arr = [];
            $warranty_arr = [];

            foreach ($imei_with_warranty as $item) {
                if (isset($item['imei'])) {
                    $imei_arr[] = $item['imei'];
                }
                if (isset($item['warranty'])) {
                    $warranty_arr[] = $item['warranty'];
                }
            }
        @endphp
        <tr data-warranties="{{ json_encode($warranties) }}" data-imei-arr="{{json_encode($imei_arr)}}" data-warranty-date="{{json_encode($warranty_arr)}}" @if(!empty($purchase_line->purchase_order_line) && !empty($common_settings['enable_purchase_order'])) data-purchase_order_id="{{$purchase_line->purchase_order_line->transaction_id}}" @endif  @if(!empty($purchase_line->purchase_requisition_line) && !empty($common_settings['enable_purchase_requisition'])) data-purchase_requisition_id="{{$purchase_line->purchase_requisition_line->transaction_id}}" @endif>
            ...
            @php
                $enable_sr_no = $purchase_line->product->enable_sr_no;
                echo $enable_sr_no;
            @endphp

            @if(session('business.enable_imei_number'))
                @php
                    $imei_number = !empty($purchase_line->imei_with_warranty) ? unserialize($purchase_line->imei_with_warranty) : null;
                @endphp
                <td class="@if($purchase_line->product->enable_sr_no == 1) purchase_imei_inputs_container @endif">
                    @if($enable_sr_no == 1)
                        @for ($i = 0; $i < $purchase_line->quantity; $i++)
                            {!! Form::text('purchases[' . $loop->index . '][imei_number]['.$i.']',  isset($imei_number[$i]['imei']) ? $imei_number[$i]['imei'] : '', ['class' => 'form-control input-sm', 'style' => 'margin-bottom: 0.5rem;']); !!}
                        @endfor
                    @else
                        {!! Form::text('purchases[' . $loop->index . '][imei_number][0]','', ['class' => 'form-control input-sm', 'style' => 'margin-bottom: 0.5rem;']); !!}
                    @endif
                </td>
            @endif
            ...
            @if($is_warranty_enabled)
                @php
                    $warranty_date = !empty($purchase_line->imei_with_warranty) ? unserialize($purchase_line->imei_with_warranty) : null;
                @endphp
                <td class="@if($purchase_line->product->enable_sr_no == 1) purchase_warranty_container @endif">
                    @if($enable_sr_no == 1)
                        @for ($i = 0; $i < $purchase_line->quantity; $i++)
                            <div style="height: 15px !important; margin-bottom: 2rem !important">
                                {!! Form::select('purchases[' . $loop->index . '][warranty_date]['.$i.']', $warranties, isset($warranty_date[$i]['warranty']) ? intval($warranty_date[$i]['warranty']) : null , ['class' => 'form-control select2', 'style' => 'width: 100% !important;', 'placeholder' => __('messages.please_select')]); !!}
                            </div>
                        @endfor
                    @else
                        <div style="height: 15px !important; margin-bottom: 2rem !important">
                            {!! Form::select('purchases[' . $loop->index . '][warranty_date][0]', $warranties, null , ['class' => 'form-control select2', 'style' => 'width: 100% !important;', 'placeholder' => __('messages.please_select')]); !!}
                        </div>
                    @endif
                </td>
            @endif
