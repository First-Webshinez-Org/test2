        ...
        <table class="table bg-gray">
          <thead>
            <tr class="bg-green">
              ...
              @if(session('business.enable_product_code'))
                <th>@lang('product.product_code')</th>
              @endif
              @if($purchase->type == 'purchase_order')
          ...
          @foreach($purchase->purchase_lines as $purchase_line)
            <tr>
              ...
              @if(session('business.enable_product_code'))
                <td>{{ $purchase_line->product->product_code }}</td>
              @endif
              @if($purchase->type == 'purchase_order')
              <td>
              ...
              @if(session('business.enable_imei_number'))
                @php
                    $imeiNumbers = is_string($purchase_line->imei_with_warranty) ? unserialize($purchase_line->imei_with_warranty) : [];
                @endphp
                <td>
                  @foreach($imeiNumbers as $imeiNumber)
                    <div>{{ $imeiNumber['imei'] }}</div>
                  @endforeach
                </td>
              @endif
              @if(session('business.enable_product_expiry'))
