		...
		<td align="center" valign="center">
			<div style="overflow: hidden !important;display: flex; flex-wrap: wrap;align-content: center;width: {{$barcode_details->width * 1}}in; height: {{$barcode_details->height * 1}}in; justify-content: center;">
				...
				<div>
					...
					{{-- Product Name --}}
					@if(!empty($print['name']))
						<span style="display: block !important; font-size: {{$print['name_size']}}px; @if(empty($print['custom_field_value'])) margin-bottom: -1rem; @endif">
							...
							@if(!empty($print['imei_number']) && !empty($page_product->imei_number))
								<span style="font-size: {{12*$factor}}px">
									 ({{$page_product->imei_number}})
								</span>
							@endif
						</span>
					@endif
					...
					{{-- Custom Field Value --}}
					@if(!empty($print['custom_field_value']))
						<span style="display: block !important; font-size: {{$print['custom_field_size']}}px; margin-bottom: -1.2rem;">
