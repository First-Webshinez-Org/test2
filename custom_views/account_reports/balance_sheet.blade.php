...
<section class="content">
    <div class="row no-print">
        <div class="col-sm-12">
            ...
            <div class="col-md-3">
                    <div class="form-group">
                        {!! Form::label('balance_sheet_date_filter', __('report.date_range') . ':') !!}
                        {!! Form::text('date_range', null, ['placeholder' => __('lang_v1.select_a_date_range'), 'class' => 'form-control', 'id' => 'balance_sheet_date_filter', 'readonly']); !!}
                    </div>
                </div>
            @endcomponent
        </div>
                    ...
                    <tr>
                        <td>
                            <table class="table">
                                ...
                                <tr>
                                    <th>@lang('report.sell_stock'):</th>
                                    <td>
                                        <input type="hidden" id="hidden_closing_stock_by_sp" class="liability">
                                        <span class="remote-data" id="closing_stock_by_sp">
                                            <i class="fas fa-sync fa-spin fa-fw"></i>
                                        </span>
                                    </td>
                                </tr>
                        ...
                        <td>
                            <table class="table" id="assets_table">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>@lang('report.purchase_stock'):</th>
                                        <td>
...
<script type="text/javascript">
    $(document).ready( function(){
        ...
        if ($('#balance_sheet_date_filter').length == 1) {
            $('#balance_sheet_date_filter').daterangepicker(dateRangeSettings, function (start, end) {
                $('#balance_sheet_date_filter').val(
                    start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format)
                );
                update_balance_sheet();
            });
            $('#balance_sheet_date_filter').on('cancel.daterangepicker', function (ev, picker) {
                $('#balance_sheet_date_filter').val('');
                update_balance_sheet();
            });
        }
    ...
    function update_balance_sheet(){
        ...
        var start_date = $('input#balance_sheet_date_filter')
                    .data('daterangepicker')
                    .startDate.format('YYYY-MM-DD');
        var end_date = $('input#balance_sheet_date_filter')
                    .data('daterangepicker')
        ...
        $.ajax({
            url: "{{action([\App\Http\Controllers\AccountReportsController::class, 'balanceSheet'])}}?start_date=" + start_date + '&end_date=' + end_date + '&location_id=' + location_id, 
            dataType: "json",
            success: function(result){
                ...
                $('span#closing_stock_by_sp').text(__currency_trans_from_en(result.closing_stock_by_sp, true));
                __write_number($('input#hidden_closing_stock_by_sp'), result.closing_stock_by_sp);
