            ...
            @slot('tool')
                ...
                @if(session('business.enable_imei_number'))
                    <div class="col-sm-2 box-tools">
                        <div id="sell_table_imei_filter" class="dataTables_filter">
                            <label>
...
$(document).ready( function(){
    ...
    sell_table = $('#sell_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            $('.footer_total_items').html(`Item- ${parseFloat(footer_total_items).toFixed(2)}`);
    ...
    $('#sell_table_imei_filter input').on('keyup', function () {
        var searchTerm = this.value.toLowerCase();

        sell_table.rows().every(function () {
            var row = $(this.node());
