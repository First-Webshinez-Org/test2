...
<section class="content">
    ...
    <!-- cards section start -->
      <!-- <div class="card-container">
        <div class="card">
          <p>Sales</p>
          <h1 id="revenueAmount"></h1>
          <p><span class="percent">5.6%</span> from previous year</p>
        </div>
        <div class="card">
          <p>Expenses</p>
          <h1 id="expenseAmount"></h1>
          <p><span class="percent">10.5%</span> from previous 244 days</p>
        </div>
        <div class="card">
          <p>Profit</p>
          <h1 id="profitAmount"></h1>
          <p><span class="percent">10.5%</span> from previous 244 days</p>
        </div>
        <div class="card">
          <p>Profit Margin</p>
          <h1 id="profitMarginAmount"></h1>
          <p><span class="percent">10.5%</span> from previous 244 days</p>
        </div>
      </div> -->
    <!-- cards section end -->
  ...
  <tfoot>
    <tr>
      <td colspan="2" id="chart-labels">Sales Figures (Units)</td> <!-- Adjust label as necessary -->
    </tr>
  </tfoot>
</table>
...
@section('javascript')
    ...
    <script src="{{ asset('js/financial_chart.js?v=' . $asset_v) }}"></script>
    @includeIf('sales_order.common_js')
    @includeIf('purchase_order.common_js')

    <script>
        function renderChart(chartId, data, categories) {
            Highcharts.chart(chartId, {
                ...
                xAxis: {
                    categories: categories, // Dynamic categories for each chart
                    visible: true, // Make sure the axis is visible
                    title: {
                        text: null // Optionally set or hide the axis title
                    }
                },
                yAxis: {
                    min: 0, // Adjust as needed
                    title: {
                        text: 'Sales', // Adjust label text as needed
                        align: 'high'
                    },
                    labels: {
                        overflow: 'justify'
                    },
                    visible: true // Adjust based on your preference
                },
                legend: {
                ...
                exporting: {
                    enabled: false // Disable chart export
                }
            });
        }

            // Example data - you would fetch or calculate this dynamically
        const branchesData = {
            'chart-A': { data: 50, categories: ['Branch A Sales'] },
            'chart-B': { data: 70, categories: ['Branch B Sales'] }
        };

            // Render charts for each branch, including categories
        Object.keys(branchesData).forEach(chartId => {
            const branch = branchesData[chartId];
            renderChart(chartId, branch.data, branch.categories);
            });
    </script>
