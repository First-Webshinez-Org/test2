	...
	<div class="modal-content">
		...
		<div class="modal-body">
			<div class="row">
				...
				@endif
				<!-- dev-nu start -->
				<div class="form-group col-xs-12 col-sm-4 @if(!in_array('types_of_service', $enabled_modules)) hide @endif">
					<label>@lang('sale.charge')</label>
						{!! Form::select("products[$row_count][line_charge]",  $charges, $charge_id, ['placeholder' => __('messages.please_select'), 'class' => 'form-control row_charge']); !!}
				</div>
				<div class="form-group col-xs-12 col-sm-4 @if(!in_array('types_of_service', $enabled_modules)) hide @endif">
					<label>@lang('sale.charge_type')</label>
						{!! Form::select("products[$row_count][line_charge_type]", ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $charge_type , ['class' => 'form-control row_charge_type']); !!}
				</div>
				<div class="form-group col-xs-12 col-sm-4 @if(!in_array('types_of_service', $enabled_modules)) hide @endif">
					<label>@lang('sale.charge_amount')</label>
						{!! Form::text("products[$row_count][line_charge_amount]", @num_format($charge_amount), ['class' => 'form-control input_number row_charge_amount']); !!}
				</div>
				@if(!empty($discount))
					<div class="form-group col-xs-12">
						<p class="help-block">{!! __('lang_v1.applied_discount_text', ['discount_name' => $discount->name, 'starts_at' => $discount->formated_starts_at, 'ends_at' => $discount->formated_ends_at]) !!}</p>
