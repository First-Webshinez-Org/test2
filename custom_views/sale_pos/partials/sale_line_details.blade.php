<table class="table @if(!empty($for_ledger)) table-slim mb-0 bg-light-gray @else bg-gray @endif" @if(!empty($for_pdf)) style="width: 100%;" @endif>
    ...
    @foreach($sell->sell_lines as $sell_line)
        <tr>
            ...
            @endif
            @if( session()->get('business.enable_imei_number') == 1 && empty($for_ledger))
                @php
                    $imei = [];
                    if(!empty($sell_line->imei_with_warranty)){
                        $imei_arr = unserialize($sell_line->imei_with_warranty);
                        foreach ($imei_arr as $item) {
                            if (isset($item['imei'])) {
                                $imei[] = $item['imei'];
                            }
                        }
