...
<tr class="product_row" data-row_index="{{$row_count}}" data-row_product="{{$product}}"  data-row_subUnits="{{json_encode($sub_units)}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		@if(!empty($so_line))
			<input type="hidden" 
		...
		@php
			...
			$charge_type = !empty($product->line_charge_type) ? $product->line_charge_type : 'fixed';
			$charge_amount = !empty($product->line_charge_amount) ? $product->line_charge_amount : 0;
			
			if(!empty($discount)) {
				$charge_type = $discount->charge_type;
				$charge_amount = $discount->charge_amount;
			}

			if(!empty($so_line) && $action !== 'edit') {
				$charge_type = $so_line->line_charge_type;
				$charge_amount = $so_line->line_charge_amount;
			}
		...
		@php
			...
			$charge_id = !empty($action) && $action == 'edit' && !empty($product->line_charge_id)  ? $product->line_charge_id : null;
			...
			if($charge_type == 'fixed') {
				$charge_amount = $charge_amount * $multiplier;
			}
		@endphp
	...
	@if( session()->get('business.enable_imei_number') == 1 || session()->get('business.enable_product_expiry') == 1)
		@php
			$imei_enabled = session()->get('business.enable_imei_number');
			$exp_enabled = session()->get('business.enable_product_expiry');
			$imei_no_line_id = '';
			if(!empty($product->imei_no_line_id)){
				$imei_no_line_id = $product->imei_no_line_id;
			}
		@endphp
		@if(!empty($product->imei_numbers) && empty($is_sales_order))
			<select class="form-control imei_number input-sm" name="products[{{$row_count}}][imei_no_line_id][]" id="products[{{$row_count}}][imei_no_line_id][]"
			 multiple @if(!empty($product->transaction_sell_lines_id)) disabled @endif>
				<option value="">@lang('lang_v1.imei_n_expiry')</option>
				@foreach($product->imei_numbers as $imei_number)
					@php
						$selected = "";
						if($imei_number->purchase_line_id == $imei_no_line_id){
							$selected = "selected";

							$max_qty_rule = $imei_number->qty_available;
							$max_qty_msg = __('lang_v1.quantity_error_msg_in_imei', ['qty'=> $imei_number->qty_formated, 'unit' => $product->unit  ]);
						}

						$expiry_text = '';
						if($exp_enabled == 1 && !empty($imei_number->exp_date)){
							if( \Carbon::now()->gt(\Carbon::createFromFormat('Y-m-d', $imei_number->exp_date)) ){
								$expiry_text = '(' . __('report.expired') . ')';
							}
						}

						//preselected imei number if product searched by imei number
						if(!empty($purchase_line_id) && $purchase_line_id == $imei_number->purchase_line_id) {
							$selected = "selected";
					...
					@endphp
					@if(!empty($imei_number->imei_number))
							@foreach($imei_number->imei_number as $i => $imn)
								<option value="{{$imei_number->purchase_line_id . '|' . $i}}" data-qty_available="{{$imei_number->qty_available}}" data-msg-max="@lang('lang_v1.quantity_error_msg_in_imei', ['qty'=> $imei_number->qty_formated, 'unit' => $product->unit  ])" {{$selected}}>
									@if(!empty($imei_number->imei_number) && $imei_enabled == 1)				   
										{{$imn}} 
										@if(!empty($warranties))
											(Warranty: {{$warranties[$imei_number->warranty_date[$i]] ?? 'No Warranty'}})
										@else
											(Warranty: No Warranty)
										@endif

									@endif 
									@if($imei_enabled == 1 && $exp_enabled == 1)
										- 
									@endif 
									@if($exp_enabled == 1 && !empty($imei_number->exp_date)) 
										@lang('product.exp_date'): {{@format_date($imei_number->exp_date)}} 
									@endif 
									{{$expiry_text}}
								</option>
							@endforeach			   
	...
	<td>
		...
		<input type="text" data-min="1" 
			...
			id="products[{{$row_count}}][quantity]"
			name="products[{{$row_count}}][quantity]" data-allow-overselling="@if(empty($pos_settings['allow_overselling'])){{'false'}}@else{{'true'}}@endif" 
			@if($allow_decimal) 
				data-decimal=1 
			@else 
				data-decimal=0 
	...
	<td>
        @if ($sub_units && !empty($product['sub_unit_ids']))
            @foreach ($sub_units as $key => $item)
				{{-- @if (in_array($key, $product['sub_unit_ids'])) --}}
					<input type="text" id="products[{{ $row_count }}][sub_unit][{{ $key }}]" name="products[{{ $row_count }}][sub_unit][{{ $key }}]" value="{{ number_format((1 / $item['multiplier']), 2) }}"
					class="form-control input_sub_unit">
					<label for="products[{{ $row_count }}][sub_unit][{{ $key }}]">{{ $item['name'] }}</label>					
				{{-- @endif --}}
            @endforeach            
        @endif
    </td>
...
<script>
	$('.imei_number').select2({
        placeholder: "@lang('lang_v1.imei_n_expiry')",
        allowClear: true,
	...
	// new added start
	var subUnits = @json($sub_units);
    var subUnitIds = JSON.parse(@json($product->sub_unit_ids));
    var rowCount = @json($row_count);
    var productId = @json($product->product_id);
	var $quantityInput = $('.input_quantity');
	var $subUnitInput = $('.input_sub_unit')

	// console.log(subUnitIds);
	// console.log(subUnits);

	$('.product_row').click(function() {
		var $clickedRow = $(this);
		subUnitIds = JSON.parse($clickedRow.data('row_product')['sub_unit_ids']);
		subUnits = $clickedRow.data('row_subunits');

		// Add change event handler to quantityInput
		$quantityInput.change(function() {
			var $input = $(this);
			var quantity = $input.val();
			var inputId = $(this).attr('id');
			var matches = inputId.match(/products\[(\d+)\]\[quantity\]/);
			if (matches && matches.length > 1) {
				var row_count = matches[1];
			}
			
			if ((subUnitIds) && (subUnits) && subUnitIds.length > 0 && Object.keys(subUnits).length > 0) {
				$.each(subUnitIds, function(id, value) {
					var multiplier = parseFloat(subUnits[value]['multiplier']);
					var subUnitNewValue = (quantity / multiplier).toFixed(2); 
					var subUnitSelector = `products[${row_count}][sub_unit][${subUnitIds[id]}]`;
					document.getElementById(subUnitSelector).value = subUnitNewValue;
				});
			}

			// var unitPriceIncTaxSelector = `products[${row_count}][unit_price_inc_tax]`;
			// var unitPriceIncTax = document.getElementById(unitPriceIncTaxSelector).value;

			// var productSubTotal = quantity * unitPriceIncTax;
			// var subTotalSelector = `products[${row_count}][sub_total]`;
			// var subTotalTextSelector = `products[${row_count}][sub_total_text]`;
			// document.getElementById(subTotalSelector).value = productSubTotal;
			// document.getElementById(subTotalTextSelector).textContent = productSubTotal;
		});



		Array.from($subUnitInput).forEach(function (elm) {
			elm.addEventListener('input', function (){
				var $input = $(this);
				var inputId = $(this).attr('id');    
				
				
				var subUnitMatches = inputId.match(/products\[(\d+)\]\[sub_unit\]\[(\d+)\]/);
				if (subUnitMatches && subUnitMatches.length > 1) {
					var row_count = subUnitMatches[1];
				}
				
				var quantitySelector = `products[${row_count}][quantity]`;
				...
				if ((subUnitIds) && (subUnits) && subUnitIds.length > 0 && Object.keys(subUnits).length > 0) {
					$.each(subUnitIds, function(id, value) {
						var multiplier = parseFloat(subUnits[value]['multiplier']);
						var subUnitSelector = `products[${row_count}][sub_unit][${subUnitIds[id]}]`;
						var subUnit = document.getElementById(subUnitSelector);
						var subUnitNewValue = (quantity / multiplier).toFixed(2); 
						if(inputId == subUnitSelector){
							var newQuantity = subUnit.value * multiplier;
							document.getElementById(quantitySelector).value = newQuantity;
							quantity = newQuantity;
						}else {
							subUnit.value = subUnitNewValue;
						}
					});
				}

				// var unitPriceIncTaxSelector = `products[${row_count}][unit_price_inc_tax]`;
				// var unitPriceIncTax = document.getElementById(subTotalSelector).value;
				// var productSubTotal = quantity * unitPriceIncTax;

				// var subTotalSelector = `products[${row_count}][${productId}]`;
