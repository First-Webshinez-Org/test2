						...
						<table class="table table-bordered table-no-top-cell-border table-slim">
							<thead>
								<tr style="background-color: #357ca5 !important; color: white !important; font-size: 15px !important" class="table-no-side-cell-border table-no-top-cell-border text-center">
									...
									@endphp
									<!-- dev-nu start -->
									@if($receipt_details->show_sku == 1)
										<td style="background-color: #357ca5 !important; color: white !important; width: 6% !important">
											SKU
										</td>
									@endif
									<!-- dev-nu end -->
									@if($receipt_details->show_product_code == 1)
										<td style="background-color: #357ca5 !important; color: white !important; width: 6% !important">
											Product Code
										</td>
									@endif
									...
									@php
										// $total_sub_units = count($receipt_details->lines->sub_unit_details) - 1;
										// $remain_sub_units = $total_sub_units;
										// $previous_mother_sub_unit_id = 0;
									@endphp
									@foreach ($receipt_details->mother_units as $key => $mother_unit)
										<td style="background-color: #357ca5 !important; color: white !important; width: 8% !important">{{ $mother_unit['name'] }}</td>
									@endforeach
									{{-- <td style="background-color: #357ca5 !important; color: white !important; width: 8% !important">Other</td> --}}
									...
									@if($receipt_details->show_charge == 1)
									<td style="background-color: #357ca5 !important; color: white !important;width: 8% !important;">
										{{$receipt_details->line_charge_label}}
									</td>
									@endif
									...
									<td style="background-color: #357ca5 !important; color: white !important;width: 14% !important;">
								...
								@foreach($receipt_details->lines as $line)
									<tr>
										...
										<td>
											...
											<!-- multiple imei with warranty -->
											@if(!empty($line['imei_with_warranty']))
												(
													@foreach ($line['imei_with_warranty'] as $item)
										...
										@php
											$total_sub_units = count($line['sub_unit_details']);
											$remain_sub_units = $total_sub_units;
											$cell_created = false;
										@endphp

										@foreach ($receipt_details->mother_units as $key1 => $mother_unit)
											@foreach ($line['sub_unit_details'] as $key2 => $sub_unit)
											@if (($sub_unit['mother_unit_id'] == $mother_unit['id']) && (($line['product_sub_units'] !== null && in_array($key2, $line['product_sub_units'])) || $key2 == $line['unit_id']))
													<td class="text-right">{{ number_format(($line['quantity'] / $sub_unit['multiplier']), 2) }}</td>
													@php
														$remain_sub_units -= 1;
														$cell_created = true;
														break;
										...
										@if($receipt_details->show_charge == 1)
										<td class="text-right">
											{{$line['total_line_charge'] ?? 0}}
											@if(!empty($line['line_charge_percent']))
												({{$line['line_charge_percent']}}%)
											@endif
										</td>
										@endif
										<!-- dev-nu start -->
					...
					<div class="col-md-6 invoice-col width-50">
						<table class="table-no-side-cell-border table-no-top-cell-border width-100 table-slim">
							<tbody>
								...
								<!-- Charge -->
								@if($receipt_details->show_charge == 1)
									@if( !empty($receipt_details->charge) )
										<tr >
											<td>
												{!! $receipt_details->charge_label !!}
											</td>

											<td class="text-right">
												(-) {{$receipt_details->charge}}
											</td>
										</tr>
									@endif
								@endif
								...
								@if($receipt_details->show_charge == 1)
									@if( !empty($receipt_details->total_line_charge) )
										<tr >
											<td>
												{!! $receipt_details->line_charge_label !!}
											</td>
											...
											<td class="text-right">
												(+) {{$receipt_details->total_line_charge}}
											</td>
										</tr>
									@endif
								@endif
								...
								<tr>
									...
									@if( !empty($receipt_details->total_line_charge) )
										@php
											...
											$total = parseCurrency($receipt_details->total);
											$total_line_charge = parseCurrency($receipt_details->total_line_charge);

											$receipt_details->total = '৳ ' . number_format($total + $total_line_charge, 2);
										@endphp
									@endif
