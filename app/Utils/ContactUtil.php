...
class ContactUtil extends Util
{
    ...
    public function getContactQuery($business_id, $type, $contact_ids = [])
    {
        $query = Contact::leftjoin('transactions AS t', 'contacts.id', '=', 't.contact_id')
            ...
            ->leftJoin(DB::raw('(SELECT transaction_id, SUM(IF(is_return = 1, -amount, amount)) AS total_paid FROM transaction_payments GROUP BY transaction_id) AS tp'), 'tp.transaction_id', '=', 't.id')
            ->where('contacts.business_id', $business_id);
        ...
        $query->select([
            ...
            't.transaction_date',
            DB::raw('COALESCE(SUM(tp.total_paid), 0) as total_paid'),
        ]);
        ...
        // $query->leftjoin('transaction_payments as tp', 'tp.transaction_id', '=', 't.id')
