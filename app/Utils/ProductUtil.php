...
class ProductUtil extends Util
{
     ...
     * @return bool
     */
    public function createSingleProductVariation($product, $sku, $purchase_price, $dpp_inc_tax, $profit_percent, $commission_percent, $selling_price, $selling_price_inc_tax, $combo_variations = [])
    {
        ...
        $variation_data = [
            ...
            'commission_percent' => $this->num_uf(isset($commission_percent) ? $commission_percent : 0),
            ...
            if (! empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $variation_data[] = [
                        ...
                        'commission_percent' => $this->num_uf(isset($v['commission_percent']) ? $v['commission_percent'] : 0),
    ...
    public function generateProductSku($string)
    {
        ...
        $sku_size = !empty($common_settings['sku_size']) ? $common_settings['sku_size'] : 4 ; 

        $sku_prefix = Business::where('id', $business_id)->value('sku_prefix');

        return $sku_prefix.str_pad($string, $sku_size, '0', STR_PAD_LEFT);
    }
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            $new_quantity = $this->num_uf($data['quantity']);
            ...
            $im = isset($data['imei_number']) ? $data['imei_number'] : 0;
            $wd = isset($data['warranty_date']) ? $data['warranty_date'] : 0;

            if (!empty($im) && !empty($wd)) {
                if (is_array($im) && is_array($wd)) {
                    foreach ($im as $index => $imei) {
                        $imei_with_warranty[] = [
                            'imei' => $imei,
                            'warranty' => $wd[$index] ?? null
                        ];
                    }
                }
            }
            $purchase_line->imei_with_warranty = !empty($imei_with_warranty) ? serialize($imei_with_warranty) : null;

            $purchase_line->imei_number = ! empty($data['imei_number']) ? serialize($data['imei_number']) : null;
            $purchase_line->warranty_date = ! empty($data['warranty_date']) ? serialize($data['warranty_date']) : null;
    ...
    public function getProductImeiDetails($business_id, $filters, $for)
    {
        $query = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                  ->join('units', 'p.unit_id', '=', 'units.id')
                  ->leftjoin('variation_location_details as vld', 'variations.id', '=', 'vld.variation_id')
        ...
        // dd($products->where('variations.sub_sku', '5686')->get()->toArray());
    ...
    {
        $stock_history_previous = $this->getVariationStockHistoryPrevious($business_id, $variation_id, request()->input('location_id'), $start_date);
        ...
        $purchase_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                ...
                ->when(!empty($start_date) && !empty($end_date), function ($q) use ($start_date, $end_date) {
                    ...
                    return $q->whereBetween('t.transaction_date', [$adjusted_start_date, $adjusted_end_date]);
                })
                //->where('t.status', 'received')
                ->where('p.business_id', $business_id)
                ->where('variations.id', $variation_id)
        ...
        $sell_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ...
            ->select(
                ...
                'sl.sub_unit_id as sub_unit_id',
            )
            ->get()->first();

        $sub_unit_id = !empty($purchase_details->sub_unit_ids) ? json_decode($purchase_details->sub_unit_ids) : [];
        ...
        $output = [
            ...
            'sub_unit_id' => $sell_details->sub_unit_id,
            'sub_unit_ids' => $purchase_details->sub_unit_ids,
            // 'base_unit_multiplier' => $units_details[0]->base_unit_multiplier,
            'base_unit_id' => $purchase_details->base_unit_id,
            ...
            // 'total_opening_stock' => $stock_history_previous[0]['stock'] ?? $purchase_details->total_opening_stock,
            'total_opening_stock' => $stock_history_previous[0]['stock'] ?? 0,
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id, $start_date = null, $end_date = null)
    {
        ...
        $stock_history = Transaction::leftjoin('transaction_sell_lines as sl',
            ...
            ->when(!empty($start_date) && !empty($end_date), function ($q) use ($start_date, $end_date) {
                $adjusted_start_date = $start_date . ' 00:00:00';
                $adjusted_end_date = $end_date . ' 23:59:59';
                return $q->whereBetween('transactions.transaction_date', [$adjusted_start_date, $adjusted_end_date]);
            })
        ...
        $stock_history_array = [];
        $stock = $stock_history_previous[0]['stock'] ?? 0;
    ...
    public function getVariationStockHistoryPrevious($business_id, $variation_id, $location_id, $start_date = null)
    {
        $stock_history = Transaction::leftjoin('transaction_sell_lines as sl',
            'sl.transaction_id', '=', 'transactions.id')
            ->leftjoin('purchase_lines as pl',
            ...
            ->when(!empty($start_date), function ($q) use ($start_date) {
                $adjusted_start_date = $start_date . ' 00:00:00';
                return $q->where('transactions.transaction_date', '<', $adjusted_start_date);
            })
            ->whereIn('transactions.type', ['sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'purchase_return', 'sell_return', 'production_sell'])
            ->select(
                'transactions.id as transaction_id',
     ...
     */
    public function getVariationStockMisMatch($business_id, $variation_id, $location_id)
    {
