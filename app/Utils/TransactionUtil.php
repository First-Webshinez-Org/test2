...
class TransactionUtil extends Util
{
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                //dev-nu start 
                // line charge amount
                // if (! empty($product['line_charge_type']) && $product['line_charge_amount']) {
                //     $charge_amount = $uf_data ? $this->num_uf($product['line_charge_amount']) : $product['line_charge_amount'];
                //     if ($product['line_charge_type'] == 'fixed') {
                //         $unit_price = $unit_price + ($charge_amount / $multiplier);
                //     } elseif ($product['line_charge_type'] == 'percentage') {
                //         $unit_price = ((100 + $charge_amount) * $unit_price) / 100;
                //     }
                // }
                //dev-nu end
                ...
                if (! empty($product['line_charge_amount'])) {
                    $line_charge_amount = $uf_data ? $this->num_uf($product['line_charge_amount']) : $product['line_charge_amount'];

                    if ($product['line_charge_type'] == 'fixed') {
                        $line_charge_amount = $line_charge_amount / $multiplier;
                    }
                }
                //dev-nu end
                ...
                $line = [
                    ...
                    'line_charge_id' => ! empty($product['line_charge']) ? $product['line_charge'] : null,
                    'line_charge_type' => ! empty($product['line_charge_type']) ? $product['line_charge_type'] : null, //dev-nu
                    'line_charge_amount' => $line_charge_amount, //dev-nu
                ...
                // imei and warrarnty related code starts   - dev-nu-start              
                $line['imei_no_line_id'] = NULL;
                $line['imei_with_warranty'] = [];
                
                if (! empty($product['imei_no_line_id'])) {
                    $indexesToRemove = []; 

                    foreach ($product['imei_no_line_id'] as $item) {
                        $imei_no_line_id_value = explode("|", $item);
    
                        // Accessing parts
                        $imei_no_line_id = $imei_no_line_id_value[0];
                        $valueId = $imei_no_line_id_value[1];
                        $value_id = intval($valueId);

                        $line['imei_no_line_id'] = $imei_no_line_id;      
          
                        $purchase_line = PurchaseLine::where('id', $imei_no_line_id)->first();
                        
                        if ($purchase_line) {
                            $imeiNumbers = !empty($purchase_line->imei_number) ?unserialize($purchase_line->imei_number) : NULL;
                            $warrantyDates = !empty($purchase_line->warranty_date) ? unserialize($purchase_line->warranty_date) : NULL;
    
                            $imeiWithWarranty = [
                                'imei' => !empty($imeiNumbers) ? $imeiNumbers[$value_id] : NULL,
                                'warranty' => !empty($warrantyDates) ?$warrantyDates[$value_id] : NULL
                            ];
                            
                            $line['imei_with_warranty'][] = $imeiWithWarranty;

                            $indexesToRemove[] = $value_id;
                        }
                    }
                }
                ...
                if (! empty($product['imei_no_line_id'])) {
                    foreach ($indexesToRemove as $index) {
                        if (!empty($imeiNumbers)) {
                            unset($imeiNumbers[$index]);
                        }
                        if (!empty($warrantyDates)) {
                            unset($warrantyDates[$index]);
                        }
                    }
    
                    // Update the purchase line with the modified arrays
                    if ($purchase_line) {
                        $purchase_line->imei_number = !empty($imeiNumbers) ? serialize(array_values($imeiNumbers)) : NULL;
                        $purchase_line->warranty_date = !empty($warrantyDates) ? serialize(array_values($warrantyDates)) : NULL;
                        $purchase_line->save();
                    }
                }

                $line['imei_with_warranty'] = !empty($line['imei_with_warranty']) ? serialize($line['imei_with_warranty']) : NULL;
                // imei and warrarnty related code ends - dev-nu-end
    ...
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {
        ...
        $sell_line->fill(['product_id' => $product['product_id'],
            ...
            'line_discount_type' => ! empty($product['line_discount_type']) ? $product['line_discount_type'] : null,
            'line_discount_amount' => $line_discount_amount,
            'line_charge_id' => ! empty($product['line_charge']) ? $product['line_charge'] : null, //dev-nu
            'line_charge_type' => ! empty($product['line_charge_type']) ? $product['line_charge_type'] : null, //dev-nu
            'line_charge_amount' => $line_charge_amount, //dev-nu
                ...
                if ($payment_amount != 0) {
                    ...
                    if ($payment['method'] == 'cheque') {
                        if (! empty($payment['cheque_date'])) {
                            $cheque_date = $uf_data ? $this->uf_date($payment['cheque_date'], true) : $payment['cheque_date'];
                        } else {
                            $cheque_date = \Carbon::now()->toDateTimeString();
                        }
                    }else{
                        $cheque_date = NULL;
                    }
                    ...
                    $payment_data = [
                        ...
                        'cheque_cleared' => isset($payment['cheque_cleared']) ? $payment['cheque_cleared'] : 0,
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        if (! empty($payment['cheque_date'])) {
            $payment['cheque_date'] = $uf_data ? $this->uf_date($payment['cheque_date'], true) : $payment['cheque_date'];
        }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            $output['item_charge_label'] = $il->common_settings['item_charge_label'] ?? '';
            ...
            $output['mother_units'] = $this->getMotherUnits($business_details->id); // added (dev_mu)

            $output['lines'] = $details['lines'];
            $output['taxes'] = [];
            $total_quantity = 0;
            $total_line_discount = 0;
            $total_line_charge = 0;//dev-nu
            $total_line_taxes = 0;
            $subtotal_exc_tax = 0;
            $unique_items = [];
            foreach ($details['lines'] as $line) {
                ...
                $total_line_charge += ($line['line_charge_uf'] * $line['quantity_uf']);//dev-nu
            ...
            $output['total_line_charge'] = ! empty($total_line_charge) ? $this->num_f($total_line_charge, true, $business_details) : 0;//dev-nu
        ...
        //show sku
        $output['show_sku'] = $il->show_sku;

        //show product code
        $output['show_product_code'] = $il->show_product_code;
        
        //show charge
        $output['show_charge'] = $il->show_charge;
        ...
        //dev-nu start
        $output['line_charge_label'] = $invoice_layout->charge_label;
        $output['charge_label'] = $invoice_layout->charge_label;
        $output['charge_label'] .= ($transaction->charge_type == 'percentage') ? ' <small>('.$this->num_f($transaction->charge_amount, false, $business_details).'%)</small> :' : '';
        //dev-nu end
        ...
        //dev-nu start
        if ($transaction->charge_type == 'percentage') {
            $charge = ($transaction->charge_amount / 100) * $transaction->total_before_tax;
        } else {
            $charge = $transaction->charge_amount;
        }
        $output['charge'] = ($charge != 0) ? $this->num_f($charge, $show_currency, $business_details) : 0;
        //dev-nu end
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $product_code = Product::find($line->product_id)->product_code;
            ...
            $line_array = [
                ...
                'product_id' => $product->id, // added 27/03/2024 (dev_ma)
                'name' => $product->name,
                ...
                'unit_id' => $product->unit_id, // added 27/03/2024 (dev_ma)
                'units' => $unit_name, 
                'product_sub_units' => $product->sub_unit_ids, // added (dev_ma)
                'sub_unit_details' => $this->getSubUnits($product->business_id, $product->unit_id, false, $product->product_id), // added 27/03/2024 (dev_ma)

                'imei_with_warranty' => !empty($line->imei_with_warranty) ? unserialize($line->imei_with_warranty) : null,
            ...
            //dev-nu start
            $line_array['line_charge'] = method_exists($line, 'get_charge_amount') ? $this->num_f($line->get_charge_amount(), false, $business_details) : 0;
            $line_array['line_charge_uf'] = method_exists($line, 'get_charge_amount') ? $line->get_charge_amount() : 0;

            if ($line->line_charge_type == 'percentage') {
                $line_array['line_charge'] .= ' ('.$this->num_f($line->line_charge_amount, false, $business_details).'%)';

                $line_array['line_charge_percent'] = $this->num_f($line->line_charge_amount, false, $business_details);
            }

            $line_array['total_line_charge'] = $this->num_f($line_array['line_charge_uf'] * $line_array['quantity_uf'], false, $business_details);
            //dev-nu end
    ...
    public function getOutputTax($business_id, $start_date = null, $end_date = null, $location_id = null, $contact_id = null)
    {
        ...
        /**
     * Gives the total customer due and supplier due in balance sheet
     *
     * @param  int  $business_id
     * @param  int  $transaction_id
     * @return array
     */
    public function getCustomerSupplierDue($business_id, $start_date = null, $end_date = null, $location_id = null, $type = null)
    {
            $contacts = Contact::where('contacts.business_id', $business_id)
                ...
                ->select(
                    DB::raw("contacts.mobile as mobile"),
                    DB::raw("contacts.address_line_1 as address"),
                    DB::raw("contacts.country as country"),
           ...
           //Check for permitted locations of a user
            ...
            if (! empty($type)) {
                $contacts->whereIn('contacts.type', [$type, 'both']);
            }

            $adjusted_start_date = $start_date . ' 00:00:00';
            $adjusted_end_date = $end_date . ' 23:59:59';

            if (!empty($start_date) && ! empty($end_date)) {
                $contacts->where('t.transaction_date', '>=', $adjusted_start_date)
                    ->where('t.transaction_date', '<=', $adjusted_end_date);
            }
    ...
    public function calculatePaymentStatus($transaction_id, $final_amount = null)
    {
        ...
        $payments_query = TransactionPayment::where('transaction_id', $transaction_id)->get();

        $paid = 1;
        foreach ($payments_query as $payment) {
            if ($payment->method == 'cheque' && $payment->cheque_cleared == 0) {
                $paid = 0;
                break;
            }
        }
        ...
        $status = 'due';
        if ($final_amount <= $total_paid && $paid == 0) {
            $status = 'pending';
        }
        if ($final_amount <= $total_paid && $paid == 1) {
            $status = 'paid';
        }
        if ($total_paid > 0 && $final_amount > $total_paid && $paid == 1) {
            $status = 'partial';
        }
            ...
            foreach ($due_transactions as $transaction) {
                ...
                if ($total_amount > 0) {
                    ...
                    $array = [
                        ...
                        // 'payment_type' => $parent_payment->payment_type,
                        ...
                        'cheque_cleared' => isset($parent_payment->cheque_cleared) ?? $parent_payment->cheque_cleared,
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            //If imei number present consider only imei number purchase line
            if (! empty($line->imei_no_line_id)) {
                $query->where('PL.id', $line->imei_no_line_id);
            }
    ...
    /**
     * Retrieves all available imei numbers of a product from variation id
     *
     * @param  int  $variation_id
     * @param  int  $business_id
     * @param  int  $location_id
     * @return bool
     */
    public function getImeiNumbersFromVariation($variation_id, $business_id, $location_id, $exclude_empty_imei = false)
    {
        $query = PurchaseLine::join(
            'transactions as T',
            'purchase_lines.transaction_id',
        ...
        // if ($exclude_empty_imei) {
        //     $query->whereRaw('(purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned) < purchase_lines.quantity');
        // } else {
        //     $query->whereRaw('(purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned) <= purchase_lines.quantity');
        // }

        $purchase_lines = $query->select('purchase_lines.id as purchase_line_id', 'imei_number',  'warranty_date', DB::raw('(purchase_lines.quantity - (purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned)) AS qty_available'))->get();
     ...
     * @return object
     */
    public function getListChequeReport($business_id)
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->select(
                    ...
                    DB::raw('(SELECT SUM(IF(TP.is_return = 1, -1 * TP.amount, TP.amount)) FROM transaction_payments AS TP WHERE TP.transaction_id = transactions.id AND TP.method = "cheque") as cheque_amount'),
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        foreach ($payments as $payment) {
            // echo $payment->cheque_cleared;
            if ($payment->transaction_type == 'opening_balance') {
                $opening_balance_paid += $payment->amount;

                if ((isset($payment['method']) && $payment['method'] == 'cheque') && isset($payment['cheque_cleared']) && $payment['cheque_cleared'] == 0) {
                    $opening_balance_paid -= $payment->amount;
                }
            }
            ...
            $ledger[] = [
                ...
                'payment_status' => $paymentTypes[$payment->method] == 'Cheque' && $payment->cheque_cleared == 0 ? 'Pending' : '',
                'total' => '',
                'cheque_cleared' => !empty($payment->cheque_cleared) ? $payment->cheque_cleared : 0,
        ...
        $total_invoice_pending = ! empty($payments) ? $payments->where('transaction_type', 'sell')->where('is_return', 0)->where('method', 'cheque')->where('cheque_cleared', 0)->sum('amount') : 0;
        ...
        $total_invoice_paid -= $total_invoice_pending;

        $total_purchase_paid = ! empty($payments) ? $payments->where('transaction_type', 'purchase')->where('is_return', 0)->sum('amount') : 0;

        $total_purchase_paid_pending = ! empty($payments) ? $payments->where('transaction_type', 'purchase')->where('is_return', 0)->where('method', 'cheque')->where('cheque_cleared', 0)->sum('amount') : 0;
        ...
        $total_purchase_paid -= $total_purchase_paid_pending;
        ...
        foreach ($ledger as $key => $val) {
            ...
            if ((isset($val['payment_method']) && $val['payment_method'] == 'Cheque') && isset($val['cheque_cleared']) && $val['cheque_cleared'] == 0) {
                $bal -= ($credit - $debit);
            }
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $inputs = $request->only(['amount', 'method', 'note', 'card_number', 'card_holder_name',
            ...
            'cheque_number','cheque_date','cheque_cleared', 'bank_account_number', ]);
        ...
        $inputs['cheque_date'] = $request->input('cheque_date', \Carbon::now()->toDateTimeString());

        if ($format_data) {
            $inputs['paid_on'] = $this->uf_date($inputs['paid_on'], true);
            $inputs['cheque_date'] = $this->uf_date($inputs['cheque_date'], true);
