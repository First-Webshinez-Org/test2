...
class TransactionSellLine extends Model
{
    ...
    {
        $charge_amount = 0;
        if (! empty($this->line_charge_type) && ! empty($this->line_charge_amount)) {
            if ($this->line_charge_type == 'fixed') {
                $charge_amount = $this->line_charge_amount;
            } elseif ($this->line_charge_type == 'percentage') {
                $charge_amount = ($this->unit_price_before_discount * $this->line_charge_amount) / 100;
            }
        }
