...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        $report_settings = empty($business->report_settings) ? $this->businessUtil->defaultReportSettings() : $business->report_settings;
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent',  'default_commission_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
            ...
            $business_details['default_commission_percent'] = ! empty($business_details['default_commission_percent']) ? $this->businessUtil->num_uf($business_details['default_commission_percent']) : 0;
            ...
            $checkboxes = ['enable_editing_product_from_purchase',
                'enable_inline_tax',
                'enable_brand', 'enable_category', 'enable_variation', 'enable_sub_category', 'enable_product_code', 'enable_price_tax', 'enable_purchase_status',
                'enable_lot_number','enable_imei_number', 'enable_commission_percent', 'enable_racks', 'enable_row', 'enable_position', 'enable_sub_units', ];

            $report_checkboxes = ['enable_profit_loss_report', 
                'enable_purchase_n_sale', 'enable_tax_report', 'enable_supplier_n_customer_report', 'enable_customer_groups_report', 'enable_stock_adjustment_report', 'enable_trending_products', 'enable_items_report', 'enable_product_purchase_report', 'enable_product_sell_report', 'enable_purchase_payment_report', 'enable_expense_report', 'enable_register_report', 'enable_sales_representative_report', 'enable_service_staff_report', 'enable_activity_log','enable_cheque_report'];
                
            foreach ($checkboxes as $value) {
                $business_details[$value] = ! empty($request->input($value)) && $request->input($value) == 1 ? 1 : 0;
            }

            foreach ($report_checkboxes as $value) {
                $business_details['report_settings'][$value] = ! empty($request->input('report_settings')[$value]) && $request->input('report_settings')[$value] == 1 ? 1 : 0;
            }
