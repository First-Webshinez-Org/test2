...
class ReportController extends Controller
{
    ...
    public function getPurchaseSell(Request $request)
    {
     ...
     /**
     * Shows product cheque report
     *
     * @return \Illuminate\Http\Response
     */
    public function getChequeReport2(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            $sells = $this->transactionUtil->getListChequeReport($business_id);
            ...
            if (request()->has('contact_type')) {
                ...
                if (! empty($contact_type)) {
                    $sells->where('contacts.type', $contact_type);
                }
            }

            //Add condition for location,used in sales representative expense report
            if (request()->has('cheque_status')) {
                $cheque_status = request()->get('cheque_status');
                if (! empty($cheque_status) && $cheque_status == 'pending') {
                    $sells->where('tp.cheque_cleared', 0);
                }

                if (! empty($cheque_status && $cheque_status == 'cleared')) {
                    $sells->where('tp.cheque_cleared', 1);
                }
            }
            ...
            if (! empty(request()->suspended)) {
                ...
                $sales = $sells->where('transactions.is_suspend', 1)
                            ->with($with)
                            ->addSelect('transactions.is_suspend', 'transactions.res_table_id', 'transactions.res_waiter_id', 'transaction.additional_notes')
                            ->get();
            ...
            $datatable = Datatables::of($sells)
                ...
                ->addColumn('total_paid', function ($row){
                    ...
                    $total_paid_html = '<span class="total-paid" data-orig-value="'.$total_paid.'">'.$this->transactionUtil->num_f($total_paid, true).'</span>';

                    return $total_paid_html;
                })
               ->addColumn('cheque_cleared', function ($row) {
                    if ($row->cheque_cleared == 1) {
                        $payment_status = 'cleared';
                        // return '<span class="label bg-light-green"> Cleared </span>';
                    } elseif ($row->cheque_cleared == 0) {
                        // return '<span class="label bg-red"> Pending </span>';
                        $payment_status = 'pending';
                    }

                    return (string) view('sell.partials.cheque_status', ['payment_status' => $payment_status, 'id' => $row->payment_id]);    
                })
                ->editColumn('cheque_date', '{{@format_datetime($cheque_date)}}')
                // ->editColumn(
                //     'payment_status',
                //     function ($row) {
                ...
                ->editColumn('payment_status', function ($row) {
                    ...
                    $paymentStatusLabel = __('lang_v1.' . $payment_status);
                    ...
                    return '<span class="label '.$bg.'">'. $paymentStatusLabel.'</span>';
                })
                ...
                ->addColumn('due', function ($row) use ($payment_types) {
                    $pending = 0;
                    foreach ($row->payment_lines as $payment) {
                        if ($payment->method == 'cheque' && $payment->cheque_cleared == 0) {
                    ...
                    $total_remaining_html = '<span class="payment_due" data-orig-value="'.$pending.'">'.$this->transactionUtil->num_f($pending, true).'</span>';

                    return $total_remaining_html;
                })
                ->addColumn('contact_type', function ($row) {
                    return '<span>'. __('sale.' . $row->contact_type) . '</span>';
                })
                ->setRowAttr([
            ...
            $rawColumns = ['amount', 'action', 'total_paid', 'payment_status','cheque_cleared', 'invoice_no', 'payment_methods', 'conatct_name','due','final_total','contact_type'];
        ...
        return view('report.cheque_report2')
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type', 'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state', ]);
            ...
            $datatable = Datatables::of($products)
                ->editColumn('stock', function ($row) {
                    $sub_unit_id = !empty($row->sub_unit_ids) ? json_decode($row->sub_unit_ids) : [];

                    $units_details = DB::table('units')
                                    ->whereIn('id', $sub_unit_id)
                                    ->select(
                    ...
                    if ($row->enable_stock) {
                        ...
                        $htmlContent = '<div>' .
                        '<span class="current_stock" data-orig-value="' . (float) $stock . 
                        '" data-unit="' . $row->unit . '"> ' . 
                        $this->transactionUtil->num_f($stock, false, null, true) . 
                        '</span>' . ' ' . $row->unit;

                        // foreach ($units_details as $sub_unit) {
                            // if ($sub_unit->allow_pieces == 1 && !empty($sub_unit->base_unit_multiplier) && $sub_unit->allow_box == 0) {
                            //     $htmlContent .= '<br><span class="current_stock" data-orig-value="' .
                            //     (float)$stock * (float)$sub_unit->base_unit_multiplier . 
                            //     '" data-unit="' . 'SF' . '"> ' . 
                            //     $this->transactionUtil->num_f((float)$stock * (float)$sub_unit->base_unit_multiplier, false, null, true) . '</span> SF';
                            // }

                            // if ($sub_unit->allow_pieces == 1 && !empty($sub_unit->base_unit_multiplier) && $sub_unit->allow_box == 1) {
                            //     $htmlContent .= '<br><span class="current_stock" data-orig-value="' .
                            //     (float)$stock / (float)$sub_unit->base_unit_multiplier . 
                            //     '" data-unit="' . 'Box(s)' . '"> ' . 
                            //     $this->transactionUtil->num_f((float)$stock / (float)$sub_unit->base_unit_multiplier, false, null, true) . '</span> Box(s)';
                            // }
                            ...
                            //     (float)$stock * (float)$sub_unit->base_unit_multiplier . 
                            //     '" data-unit="' . 'PS' . '"> ' . 
                            //     $this->transactionUtil->num_f((float)$stock * (float)$sub_unit->base_unit_multiplier, false, null, true) . 
                            //     '</span> PS';
                            // }
                        ...
                        // return  '<div><span class="current_stock" data-orig-value="'.(float) $stock.'" data-unit="'.$row->unit.'"> '.$this->transactionUtil->num_f($stock, false, null, true).'</span>'.' '.$row->unit. '<span class="current_stock" data-orig-value="'.(float) $converted_stock.'" data-unit="'.$converted_unit.'"> '.$this->transactionUtil->num_f($converted_stock, false, null, true).'</span>'.' '.$converted_unit. '</div>';
                    } else {
                        return '--';
                    }
                })
    ...
    public function getproductPurchaseReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = PurchaseLine::join(
                ...
                't.id'
                    ...
                    ->select(
                        ...
                        't.tax_amount',
                        DB::raw('(CASE WHEN t.discount_type = "percentage" THEN t.total_before_tax * t.discount_amount / 100 ELSE t.discount_amount END) AS total_discount'),
                    )
            ...
            return Datatables::of($query)
                ...
                ->editColumn('discount_amount', function ($row) {
                    return '<span class="discount_amount" data-orig-value="'.$row->total_discount.'">'.
                    $this->transactionUtil->num_f($row->total_discount, true).'</span>';
                })
                ->editColumn('tax', function ($row) {
                    return $this->transactionUtil->num_f($row->tax_amount, true)
                     .'<br>'.'<span data-orig-value="'.$row->tax_amount.'" 
                     class="tax" data-unit="'.$row->tax.'"></span>';
                })
                ...
                ->rawColumns(['ref_no', 'unit_purchase_price', 'subtotal', 'purchase_qty', 'quantity_adjusted', 'supplier','discount_amount', 'tax'])
                ->make(true);
        }
                ...
                ->editColumn('product_name', function ($row) {
                 ...
                 ->editColumn('invoice_no', function ($row) {
                     return '<a data-href="'.action([\App\Http\Controllers\SellController::class, 'show'], [$row->transaction_id])
                            .'" href="#" data-container=".view_modal" class="btn-modal invoice_no">'.$row->invoice_no.'</a>';
                 })
                ->editColumn('transaction_date', '{{@format_datetime($transaction_date)}}')
    ...
    /**
     * Shows product imei report
     *
     * @return \Illuminate\Http\Response
     */
    public function getImeiReport2(Request $request)
    {
        if (! auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $products = $query->select(
                ...
                DB::raw("(COALESCE((SELECT SUM(quantity - quantity_returned) FROM purchase_lines as pls WHERE pls.variation_id = v.id AND pls.imei_number = pl.imei_number), 0) - SUM(COALESCE((tspl.quantity - tspl.qty_returned), 0))) as stock"),

                DB::raw('COALESCE(SUM(IF(t.type = "sell_transfer" OR tspl.sell_line_id IS NULL, 0, (tspl.quantity - tspl.qty_returned)) ), 0) as total_sold'),

                // DB::raw('COALESCE(SUM(IF(tspl.stock_adjustment_line_id IS NULL, 0, tspl.quantity ) ), 0) as total_transfered'),
            ...
            ->whereNotNull('pl.imei_number')
            ->groupBy('v.id', 'pl.imei_number');
            ...
            return Datatables::of($products)
               ...
               ->editColumn('imei_number', function ($row) {
                    $innerHtml = '';
                    if (!empty($row->imei_number)) {
                        $imeiNumbers = unserialize($row->imei_number);
                        foreach ($imeiNumbers as $imn) {
                            $innerHtml .= isset($imn) ? $imn . '<br>' : "" . '<br>';
                        }
                    }
                    return $innerHtml;
                })
               ->editColumn('imei', function ($row) {
                    if (!empty($row->imei_number)) {
                        return unserialize($row->imei_number);
                    }
                    return [];
                })
                ->editColumn('warranty_date', function ($row) {
                    $business_id = session()->get('user.business_id');

                    $warranties = Warranty::forDropdown($business_id);
                    
                    $innerHtml = '';
                    if (!empty($row->warranty_date)) {
                        $warrantyDates = unserialize($row->warranty_date);
                        foreach ($warrantyDates as $wd) {
                            $innerHtml .= isset($warranties[$wd]) ? $warranties[$wd]  . '<br>' : 'No Warranty' . '<br>';
                        }
                    }
                    return $innerHtml;
                })
                ...
                ->editColumn('total_transfered', function ($row) {
                    if ($row->total_transfered) {
                        return '<span data-is_quantity="true" class="display_currency total_transfered" data-currency_symbol=false data-orig-value="'.(float) $row->total_transfered.'" data-unit="'.$row->unit.'" >'.(float) $row->total_transfered.'</span> '.$row->unit;
                    } else {
                        return '0'.' '.$row->unit;
                    }
                })
                ...
                ->setRowAttr([
                    'data-href' => function ($row) {
                        if (auth()->user()->can('purchase.view')) {
                            return  action([\App\Http\Controllers\ReportController::class, 'show'], [$row->id]);
                        } else {
                            return '';
                        }
                    }, ])
                // ->rawColumns(['final_total', 'ref_no', 'name'])
                ->rawColumns(['exp_date', 'stock', 'total_sold', 'total_adjusted', 'total_transfered','imei_number','warranty_date'])
                ->make(true);
        }
        ...
        return view('report.imei_report2')
            ->with(compact('business_locations', 'suppliers', 'orderStatuses','categories', 'brands', 'units'));
    }
     ...
     * @return \Illuminate\Http\Response
     */
    public function getImeiReport(Request $request)
    {
        if (! auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $products = $this->productUtil->getProductImeiDetails($business_id, $filters, $for);
            //To show stock details on view product modal
            if ($for == 'view_product' && ! empty(request()->input('product_id'))) {
                $product_stock_details = $products;
        ...
        return view('report.imei_report')
            ->with(compact('categories', 'brands', 'units', 'business_locations', 'show_manufacturing_data'));
    }
