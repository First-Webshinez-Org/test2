        ...
        if (! empty($product) && $product->enable_stock == 1) {
            ...
            foreach ($transactions as $transaction) {
                foreach ($transaction->purchase_lines as $purchase_line) {
                    ...
                    $purchase_lines[$purchase_line->variation_id][$k]['imei_number'] = !empty($purchase_line->imei_number) ? unserialize($purchase_line->imei_number) : null;

                    $purchase_lines[$purchase_line->variation_id][$k]['warranty_date'] = !empty($purchase_line->warranty_date) ? unserialize($purchase_line->warranty_date) : null;
                    ...
                    if (array_key_exists($location_id, $locations)) {
                        foreach ($value as $vid => $purchase_lines_data) {
                            ...
                            foreach ($purchase_lines_data as $k => $pl) {
                                ...
                                $im = isset($pl['imei_number']) ? $pl['imei_number'] : null;
                                $wd = isset($pl['warranty_date']) ? $pl['warranty_date'] : null;
                                ...
                                $imei_with_warranty = !empty($imei_with_warranty) ? serialize($imei_with_warranty) : null;

                                $imei_number = null;
                                if (! empty($pl['imei_number'])) {
                                    $imei_number = serialize($pl['imei_number']);
                                }

                                $warranty_date = null;
                                if (! empty($pl['warranty_date'])) {
                                    $warranty_date = serialize($pl['warranty_date']);
                                }
                                ...
                                if (! is_null($purchase_line)) {
                                    ...
                                    $purchase_line->imei_number = $imei_number;
                                    $purchase_line->warranty_date = $warranty_date;
                                    $purchase_line->imei_with_warranty = $imei_with_warranty;
